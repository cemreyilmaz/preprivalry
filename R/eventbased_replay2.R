...
# replay score
# ---------------------------------------------------------------------------- #
#' Replay Score calculated as event-based when the task is dominance or transition
#'
#' @param data list
...
eventbased_replay2 <- function(data, expType="None", dominance_key = 18){
  ...
  for(t in 1:length(exp)){
    ...
    if(dim(trial_key)[1] > 1){ # if any key event was recorded
      ...
      key_events[key_events==dominance_key] <- 1 # dominance
      key_events[key_events!=dominance_key] <- 0 # transition
      trial_transperancy[trial_transperancy == -1] <- 1 # dominance
