...
test_that("extract_exp returns data.frame", {expect_true(is.data.frame(
  extract_exp(read_rivdata(directory,"Gratings","s001",1))
))})

test_that("extract_key returns data.frame", {expect_true(is.data.frame(
  extract_key(read_rivdata(directory,"Gratings","s001",1))
))})

exp<-extract_exp(read_rivdata(directory,"Gratings","s001",1))
key<-extract_key(read_rivdata(directory,"Gratings","s001",1))
...
test_that("preprocessing_session returns a list", {
  expect_true(is.list(preprocessing_session(directory,c("Gratings","Images","GratingImage"),"s001",1)))
})

test_that("preprocessing_subject returns a list", {
  expect_true(is.list(preprocessing_subject(directory,"Gratings","s001",3)))
})

test_that("read_riv returns a list", {
  expect_true(is.list(read_rivdata(directory,"Gratings","s001",1)))
})
...
test_that("replay_score returns numeric", {expect_true(is.numeric(
  replay_score(rescale_replay_data(directory,"Gratings","s001",3))
))})

test_that("rescale_replay_data returns data.frame", {expect_true(is.data.frame(
  rescale_replay_data(directory,"Gratings","s001",3)
))})
